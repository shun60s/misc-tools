...
class Net2(nn.Module):    # convert 40 to 10,  small network
    def __init__(self,):
        super(Net2, self).__init__()
        self.fc1 = nn.Linear(40,10)  # z_dim =2 * 10 digits = 40  to digit 10
...
class Net(nn.Module):
    def __init__(self,device):
        ...
        if 0:  # if 1, use basic funcs
            self.define_r3x3(device)
            self.fc1 = nn.Linear(self.lout * self.r3x3.size(0), 40)  # z_dim =2 * 10 digits = 40
        else:
            self.fc1 = nn.Linear(self.lout * 3 * 3, 40)
...
def train(args, model, model2, device, train_loader, optimizer, optimizer2,  epoch):
    model.train()
    model2.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        target2 = target // 4  # 4現象分を1種類としてあつかう
...
def test(model, model2, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            target2 = target // 4  # 4現象分を1種類としてあつかう
            test_loss2 += F.nll_loss(output2, (target2), reduction='sum').item()  # sum up batch loss
    ...
    print('\nTest set: Average loss: {:.4f}, {:.4f}, Accuracy: {}/{} ({:.3f}%) {}/{} ({:.3f}%)\n'.format(
        test_loss, test_loss2,    correct, len(test_loader.dataset),
...
def main():
    ...
    # add 40 to 10 small model of Net2
    model  = Net(device).to(device)
    model2 = Net2().to(device)
    
    model_path  ="mnist_cnn.pt"
    model_path2 ="mnist_40to10.pt"
    if args.load_model:
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model, model2, device, train_loader, optimizer, optimizer2, epoch)
    ...
    if args.save_model:
        ...
        """
    if 0: # set 1 to show incorrect prediction image with binarization
        show_incorrect_image(model, device, test_loader)
    
    if 0: # set 1 to show heat map of weights of last fully-connected layer, fc1(10, ...
        show_heat_map(model)
